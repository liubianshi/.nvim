" Perldoc Viewer ======================================================== {{{1
" Adapted from: hotchpotch/perldoc-vim: Perldoc plugin for vim
"   https://github.com/hotchpotch/perldoc-vim/blob/master/plugin/perldoc.vim
...
function! s:PerldocWord(word)
  if s:ClassExist(a:word)
    ...
    call s:ShowCmd('0read!perldoc -ovim -T ' . a:word)
  elseif s:FuncExist(a:word)
    let s:mode = ''
    let s:last_word = a:word
    call s:ShowCmd('0read!perldoc -ovim -f ' . a:word)
  elseif s:VarsExist(a:word)
    let s:mode = ''
    let s:last_word = a:word
    call s:ShowCmd('0read!perldoc -ovim -v ' . a:word)
  else
...
let s:perlpath = ''
function! perldoc#PerldocComplete(ArgLead, CmdLine, CursorPos)
  if len(s:perlpath) == 0
    try
