...
nmap(
  "<A-M>",
  "Shift cursor To:",
  "<cmd>call utils#ShiftLine(line('.') + 1, col('.') - 1)<cr>"
)
imap("<A-M>", "Move cursor to:", "<esc>:call utils#MoveCursorTo('')<cr>a")
vimkey(
  "<A-m>",
  "Move cursor To:",
  "<cmd>call utils#MoveCursorTo()<cr>",
  { mode = { "n", "i" } }
)
...
vmap(
  ...
  ":<C-u>call utils#VisualSelection('', '')<CR>/<C-R>=@/<CR><CR>"
)

-- Fold and add symbol -------------------------------------------------- {{{1
nmap(
  "<leader>z-",
  "Add - as seporator with Fold marker",
  '<cmd>call utils#AddFoldMark("-")<cr>'
)
nmap(
  "<leader>z=",
  "Add = as seporator with Fold marker",
...
nmap(
  "<leader>eo",
  "Neovim Options",
  "<cmd>edit ~/.config/nvim/lua/options.lua<cr>"
)
...
nmap(
  "<leader>et",
  "Open Plan to Do",
  "<cmd>edit +ToggleZenMode ~/Documents/Writing/plantodo.norg<cr>zt"
)
nmap(
  "<leader>eT",
  "Open Plan to Do",
  "<cmd>edit +$ ~/Documents/Writing/todo.norg<cr>zt"
)
